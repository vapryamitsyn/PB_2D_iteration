    ...
    USE BOX
    IMPLICIT NONE    
    REAL(8),DIMENSION(NT,0:N_comp-1),PUBLIC :: RHO,W
    end module
    PROGRAM  PB_2D_1
    USE BOX
    use fields
    ...
    CHARACTER(LEN=16):: HEADXX(27)=["Ns","M_poly","Qp","B2","M_+", "M_-","Mpoly","MI+","MI-","TSp","TS+","TS-","TS","Entlp","EsE","FE","Zx","Qz","Q+","Q-","Ea+","Ea-","Xads","F_DS_P","FDS_C","L_D","Epsilion_Sol"]

    CHARACTER(*), PARAMETER :: APFN="aparticles.lst", PPFN="ppositions.start", SFN="scenariop.lst" ,SAFN="scenarioa.lst"
    ...
    NAMELIST /SCENARIOA/ M2T,ALPHA0,ALPHAM,ALPHAS
    ...
    !ZMINUS=M_MINUS
    !ZX=1.
    L_D=1/SQRT((M_PLUS+M_MINUS)*LVI*EPSCI(M2T*RHO0*M_POLY*LVI,0._8))
    Epsilion_Sol=LBX/(EPSCI(M2T*RHO0*M_POLY*LVI,0._8))
    ...
    DO WHILE(M_POLY<=MPMAX)
        ...
        DO WHILE(alphaPbulk<=ALPHAM)
            ...
            DO LS=1,NDSN
                ...
                DO K=1,IMAX
                    DO J=1,100
                        ...
                        CALL ITERATE_PB(rho(:,0),max(1,(PBIT-1)/LS),QTEST)
                    END DO
                    CALL ITERATE_PB(rho(:,0),max(1,(PBIT-1)/LS),QTEST,DELTAPHI_MAX,ESE)
                ...
                RESXX=[REAL(NS,8),M_POLY,Qpx,B2,M_PLUS,M_MINUS,MPOLY,MIPLUS,MIMINUS,TSP,TSPLUS,TSMINUS,TS,ENTLP,ESE,FE,ZX,QZ,Qplus,Qminus,EAPLUS,EAMINUS,XADS,ALPHAPS,ALPHBETCS,L_D,Epsilion_Sol]

                CALL APPEND_DAT("results.tsv",dat=RESXX,N=27)
            END DO
            ...
            IF(save_SNAP_psi)THEN
                ...
                CALL DUMP_DENS(RHOIPLUS,"Mp"//NUMCHAR(INT(M_POLY))//"Al"//NUMCHAR(INT(1000000*alphaPbulk))//"plus.dat")
                CALL DUMP_DENS(RHOIMINUS,"Mp"//NUMCHAR(INT(M_POLY))//"Al"//NUMCHAR(INT(1000000*alphaPbulk))//"minus.dat")
                ...
                CALL DUMP_DENS(RHOIMINUS,"Mp"//NUMCHAR(INT(M_POLY))//"Al"//NUMCHAR(INT(1000000*alphaPbulk))//"Qp.dat")
        ...
        Epsilion_Sol=LBX/(EPSCI(M2T*RHO0*M_POLY*LVI,0._8))
        Print '("Ions Debye-Huckel length is:",G11.3," Epsilon=",G11.3," fi=",G11.3)', &
            Real(L_D,4),REAL( Epsilion_Sol,4),REAL(M2T*RHO0*M_POLY*LVI,4)
        !CALL MAKE_IQ2(IQ2)
    END DO
    ...
    INQUIRE (FILE=SAFN,EXIST=exist)
    if(exist)then
        OPEN(UNIT=10,FILE=SAFN,FORM ='FORMATTED', ACTION='READ',STATUS='UNKNOWN',DELIM='APOSTROPHE')
        READ(10,SCENARIOA)
        CLOSE(10)
        PRINT START
    ELSE
        ...
        ALPHAM=1.
        ALPHAS=.3
        OPEN(UNIT=10,FILE=SAFN,FORM ='FORMATTED', ACTION='WRITE',STATUS='UNKNOWN',DELIM='APOSTROPHE')
        WRITE(10,SCENARIOA)
    END IF
    M_PLUS=M_POLY*M2T+M_MINUS
...
3   STOP 'STARTED FILE ERROR '
    ...
    ZPA=PLX*(1._8-ALPHAPBULK)/ALPHAPBULK
    LZX=log(ZPA/PLX)
    m2=M2T/alphaPbulk
    PRINT*,"AlphapBulk=",Real(ALPHAPBULK,4)," M2=",Real(M2,4)," pKa_P=",Real(-LOG10(ZPA),4)
