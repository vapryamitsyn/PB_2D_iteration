    ...
    NAMELIST /STARTPOLYMER/   RG,M_POLY,B2,M2, M_PLUS, M_MINUS,PKA_P,pH,RHO0

    NAMELIST /START/ IMAX,DELTA_W,DELTA_PSI,LBX,LX,PBIT, & !LBX is the 4*pi*Bjerrum_length
    SAVE_SNAP_RHO,SAVE_SNAP_PSI                 

    NAMELIST /STARTPARTICLES/ MPARTICLES, R_PARTICLE,RU_PARTICLE,U_PARTICLE,U0,APN,BPN,PKA_C,PKB_C,SAVE_SNAP_P
    ...
    REAL(8) :: RESXX(29),L_D,Epsilion_Sol
    real(8) :: psir(0:nx/2-1),rhor(0:nx/2-1),rhoir(0:nx/2-1)
    real(8):: alphapbulk,wbulk,alphaFEbulk,LZX
    !integer::ix,iy,iz,ir

    CHARACTER(LEN=16):: HEADXX(29)=["Ns","Qc","M_poly","Qp","B2","M_+", "M_-","Mpoly", &
    "MI+","MI-","TSp","TS+","TS-","TS","Entlp","Eads","EsE","FE","Zx","Qz","Q+","Q-","Ea+","Ea-","Xads","F_DS_P","FDS_C","L_D","Epsilion_Sol"]
    ...
    IF(READ_PARTICLE()) THEN  
        PRINT*, "PARTICLES CONFIGURATION IS RED"
    ELSE
        !STOP "Particles configuration must be created before running the program"  
        PRINT*, "Particles configuration created"
    END IF 



    CALL  SAVE_PARTICLES()
    PRINT*, "PARTICLES CONFIGURATION SAVED"  
    ...
    DO WHILE(M_POLY<=MPMAX)
        ...
        DO LS=1,NDSN
            ...
            DO K=1,IMAX
                ...
                ELSE
                    ...
                    DO I=1,NT
                        WX=(B2*MPOLY*RHO(I)-M2*ALPHAP(I)*PSI(I)-0.5*LBX*EPS_WI*EPS_CP*M2*RHO0*E2(I)/(EPS_I(I)**2)+WP(I)+&
                ...
                FE=(ENTLP+EsE+EADS+EAPLUS+EAMINUS-TS)-FE0
                ...
                PRINT '("Qc=",G10.3," Qp=",G10.3," Sp=",G10.3," S+=",G9.2," S-=",G9.2," S=",G10.3,"DP=",G11.3," DC=",G11.3)',  & 
                Qc/MPARTICLES,Qpx,TSP,TSPLUS,TSMINUS,TS,-ALPHAPS,-ALPHBETCS
                PRINT '("EsE=",G10.3,"Entlp=",G10.3," Eads=",G10.3," Ea+=",G10.3," Ea-=",G10.3," F=",G13.6)',  & 
                ESE, ENTLP,EADS,EAPLUS,EAMINUS,FE
            ...
            RESXX=[REAL(NS,8),Qc/MPARTICLES,M_POLY,Qpx,B2, &
            M_PLUS,M_MINUS,MPOLY,MIPLUS,MIMINUS,TSP,TSPLUS,TSMINUS,TS,ENTLP,EADS,ESE,FE,ZX,QZ,Qplus,Qminus,EAPLUS,EAMINUS,XADS, &
            ALPHAPS,ALPHBETCS,L_D,Epsilion_Sol]

            CALL APPEND_DAT("results.tsv",dat=RESXX,N=29)
        END DO
        ...
        IF(MPARTICLES==1)THEN
            CALL PSI_R(PSI,PSIR)
            OPEN(UNIT=198,FILE="PsiR"//"mp"//NUMCHAR(INT(M_POLY))//".dat",FORM ='FORMATTED',ACTION='WRITE',STATUS='REPLACE')
            WRITE(198,'(" ", G30.22E3," ")')  PSIR
            CLOSE(198)

            CALL PSI_RX(RHO,RHOR)
            OPEN(UNIT=199,FILE="RhoR"//"mp"//NUMCHAR(INT(M_POLY))//".dat",FORM ='FORMATTED',ACTION='WRITE',STATUS='REPLACE')
            WRITE(199,'(" ", G30.22E3," ")')  rhor
            CLOSE(199)

            CALL PSI_RX(RHOIPLUS,RHOIR)
            OPEN(UNIT=200,FILE="RhoIPR"//"mp"//NUMCHAR(INT(M_POLY))//".dat",FORM ='FORMATTED',ACTION='WRITE',STATUS='REPLACE')
            WRITE(200,'(" ", G30.22E3," ")')  rhoir
            CLOSE(200)
            
            CALL PSI_RX(RHOIMINUS,RHOIR)
            OPEN(UNIT=201,FILE="RhoIMR"//"mp"//NUMCHAR(INT(M_POLY))//".dat",FORM ='FORMATTED',ACTION='WRITE',STATUS='REPLACE')
            WRITE(201,'(" ", G30.22E3," ")')  rhoir
            CLOSE(201)
            
            CALL PSI_RX(ALPHAP,RHOIR)
            OPEN(UNIT=202,FILE="AlphaPR"//"mp"//NUMCHAR(INT(M_POLY))//".dat",FORM ='FORMATTED',ACTION='WRITE',STATUS='REPLACE')
            WRITE(202,'(" ", G30.22E3," ")')  rhoir
            CLOSE(202)
            ...
            CALL PSI_RX(RHOIMINUS,RHOIR)
            OPEN(UNIT=203,FILE="QpR"//"mp"//NUMCHAR(INT(M_POLY))//".dat",FORM ='FORMATTED',ACTION='WRITE',STATUS='REPLACE')
            WRITE(203,'(" ", G30.22E3," ")')  rhoir
            CLOSE(203)
            
            CALL PSI_RX(RHOQ,RHOIR)
            OPEN(UNIT=204,FILE="RhoQR"//"mp"//NUMCHAR(INT(M_POLY))//".dat",FORM ='FORMATTED',ACTION='WRITE',STATUS='REPLACE')
            WRITE(204,'(" ", G30.22E3," ")')  rhoir
            CLOSE(204)
        ...
        IF(save_SNAP_psi)THEN
            ...
            !$OMP PARALLEL  DO DEFAULT(SHARED) PRIVATE(I)      
            DO I=1,NT            
                RHOIMINUS(I)=((BPN*BETAC(I))-(APN*ALPHAC(I)))*QP(I)
            ENDDO
            !$OMP END PARALLEL DO 
            CALL DUMP_DENS(RHOIMINUS,"Mp"//NUMCHAR(Int(M_POLY))//"Qc.dat")
        END IF
...
3   STOP 'STARTED FILE ERROR '
    ...
    !----------------------------------------------------------------------------------------------------------------------------
    LOGICAL FUNCTION READ_PARTICLE() RESULT(EXIST)
    real(8)::DELTA
    INTEGER :: IVAR
    ...
    ELSE
        ...
        EPS_CP=1.
        EPS_CC=1.  
        OPEN(UNIT=10,FILE=EPFN,FORM ='FORMATTED', ACTION='WRITE',STATUS='REPLACE',DELIM='APOSTROPHE')
        WRITE(10,EPSILONS)
        CLOSE(10)
        STOP 'Epsilons  list created'
    END IF


    INQUIRE (FILE=APFN,EXIST=EXIST)
    IF(EXIST)THEN
        OPEN(UNIT=10,FILE=APFN,FORM ='FORMATTED', ACTION='READ',STATUS='UNKNOWN',DELIM='APOSTROPHE')
        READ(10,STARTPARTICLES)
        CLOSE(10)
    ...
    ELSE
        ...
        PKB_C=5.
        SAVE_SNAP_P=.FALSE.
        OPEN(UNIT=10,FILE=APFN,FORM ='FORMATTED', ACTION='WRITE',STATUS='REPLACE',DELIM='APOSTROPHE')
        WRITE(10,STARTPARTICLES)
        CLOSE(10)
        STOP 'paricle parametes list created'
    END IF
    ZCA=10.**(-PKA_C)
    ZCB=10.**(-PKB_C)
    DELTA=((1.+SQRT(1.+U0/U_PARTICLE)))**(-1./3.)
    DELTAM1=1._8-DELTA
    RMD=RU_PARTICLE*DELTA
    RPX2=4.*(R_PARTICLE+RU_PARTICLE)**2

    !    PQN=MPARTICLES*Q_PARTICLE

    ALLOCATE(PARTICLE(MPARTICLES))

    INQUIRE(FILE=PPFN , ERR=1 , IOSTAT=ivar ,  EXIST=EXIST)

    IF(EXIST) then
        OPEN(UNIT=299,FILE=PPFN,FORM ='FORMATTED',ACTION='READ',STATUS='OLD')
        READ(299,*,ERR=1,END=1)  PARTICLE
        CLOSE(299)
        PARTICLE=ROUNDV(PARTICLE)
    ELSE
        !STOP "particles coordinates file not found"
        CALL MAKE_PARTICLES()
    END IF
    CALL INIT_PARTICLE() 
    RETURN
1   PRINT*,'I/O error in ' ,PPFN, ' IOSTAT=',IVAR
    STOP "START ERROR"     
    END function READ_PARTICLE
    !-----------------------------------------------------------------------------------------------------
    SUBROUTINE SAVE_particles()
    !INTEGER :: IVAR

    OPEN(UNIT=10,FILE=APFN,FORM ='FORMATTED', ACTION='WRITE',STATUS='UNKNOWN',DELIM='APOSTROPHE')
    WRITE(10,STARTPARTICLES)
    CLOSE(10)


    OPEN(UNIT=299,FILE=PPFN,FORM ='FORMATTED',ACTION='WRITE',STATUS='REPLACE')
    WRITE(299,*)  PARTICLE
    CLOSE(299)

    IF(save_SNAP_P) call dump_dens(QP,"snap_qp.dat")

    RETURN

2   STOP "ERROR in outparticles"
    ...
    INTEGER::I    
    EPLUS=0.; EMINUS=0. 
    RHON=0._8 ; EW=0.; Eads=0.
    F2=HUGE(1.);F1=-F2
    RHO1=F1; RHO2=F2;  ENTLP=0.
    EAPLUS=0.;EAMINUS=0.
    ALPHAPS=0.; ALPHBETCS=0.
    !$OMP PARALLEL DEFAULT(SHARED) PRIVATE(I,PS,RHP,WI,ROIP,ROIM) 
    !$OMP DO REDUCTION(MAX:F1,RHO1) REDUCTION(MIN:F2,RHO2) REDUCTION(+:RHON,EW,ENTLP,Eads) 
    DO I=1,NT
        RHP=RHO(I)
        PS=PSI(I)       
        RHON=RHON+RHP
    ...
    DO I=1,NT
        ...
        EAPLUS=EAPLUS+ROIP*WI
        EMINUS=EMINUS+XLNX(ROIM)
        EAMINUS=EAMINUS+ROIM*WI
    ENDDO
    !$OMP END  DO 
    ...
    ENDDO
    !$OMP END  DO 
    !$OMP DO REDUCTION(+:ALPHBETCS)
    DO I=1,NT
        ALPHBETCS=ALPHBETCS+&
        (APN*(XLNX(ALPHAC(I))+XLNX(1.-ALPHAC(I))+ALPHAC(I)*LOG(ZCA/plx))+&
        BPN*(XLNX(BETAC(I))+XLNX(1.-BETAC(I))+BETAC(I)*LOG(ZCB*plx)))*QP(I)
    ENDDO
    !$OMP END  DO 
