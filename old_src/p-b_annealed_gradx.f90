    ...
    MAIN:  DO, K=1,N_IT
        ...
        DO I=1,NT
            RHOQ(I)=NORMPLUS*RHOIPLUS(I)-NORMIMINUS*RHOIMINUS(I)-MPOLY*M2*ALPHAP(I)*RHO(I) &
                +(bpn*BETAC(I)-apn*ALPHAC(I))*QP(I)
        ENDDO
        !$OMP END DO
        call GPDOTGPHI(LEPSILON,PSI,GEGP,E)
        !$OMP  DO
        DO I=1,NT
            EXPSI(I)=LBX*RHOQ(I)*EPS_I(I)+GEGP(I)
        ENDDO
        !$OMP END DO
        !$OMP END PARALLEL
    ...
    !$OMP DO    REDUCTION(+:DQ)
    DO J=1,NT
        DQ=DQ+(BPN*BETAC(J)-APN*ALPHAC(J))*QP(J)
    END DO
    !$OMP END DO
    !$OMP END PARALLEL
    QPX=QPX*LVN
    ...
    SUBROUTINE gpdotgphi(EPSI,PSI,gegp,E) 
